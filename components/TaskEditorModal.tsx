  ...
  useEffect(() => {
    if (isOpen) {
        if (task) {
          ...
          setCategoryId(catId);
          // Ensure category input is populated even for new tasks if category is preset
      ...
      <div className="bg-white rounded-2xl w-full max-w-[320px] overflow-hidden border border-stone-200 shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center px-5 py-4 bg-stone-50 border-b border-stone-100 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-black text-[13px] text-stone-800 tracking-tight">{simplified ? '新建待办模板' : '任务模板设定'}</h3>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-1">
                ...
                <div className="relative group">
                    ...
                    <input type="text" value={name} onChange={(e) => {setName(e.target.value);}} className="w-full pl-9 pr-4 py-2 bg-stone-50 border border-stone-100 rounded-lg focus:outline-none focus:bg-white focus:border-stone-900 transition-all font-bold text-xs text-stone-800 shadow-sm" placeholder="你想做什么？" autoFocus />
                </div>
            </div>

            <div className="space-y-1">
                <label className="text-[9px] font-black text-stone-400 uppercase tracking-widest ml-1">选择分类</label>
                <div className="flex flex-wrap gap-2">
                    <button
                        type="button"
                        onClick={() => { setCategoryId(''); setCategoryInput(''); }}
                        className={cn(
                    ...
                    {objectives.map(obj => (
                        ...
                        >
                            <div className={cn("w-1.5 h-1.5 rounded-full", categoryId === obj.id ? "bg-white" : "")} style={{ backgroundColor: categoryId === obj.id ? undefined : obj.color }} />
                            {obj.title}
                        </button>
                    ))}
                    {onAddObjective && (
                         <button 
                            ...
                            className="px-4 py-2 rounded-lg border border-dashed border-stone-200 bg-stone-50 text-stone-400 hover:text-stone-600 hover:border-stone-300 transition-all text-[10px] font-black flex items-center gap-1"
                        >
                            <Plus size={12} /> 新建
                        </button>
                    )}
