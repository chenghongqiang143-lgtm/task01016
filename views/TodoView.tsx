...
export const TodoView: React.FC<TodoViewProps> = ({
  ...
  const MonthView = () => {
    ...
    return (
        <div className="flex flex-col bg-stone-50">
            <div className="grid grid-cols-7 border-b border-stone-200 bg-white shadow-sm">
                {['一', '二', '三', '四', '五', '六', '日'].map(d => (
                ...
                {days.map(day => {
                    ...
                    return (
                        <div 
                            ...
                            onClick={() => onDateChange?.(day)}
                            className={cn(
                                "aspect-[1.2] border-b border-r border-stone-200/50 flex flex-col items-center justify-start py-2 gap-1 cursor-pointer active:bg-stone-100",
  ...
  const renderTodoListItems = () => {
    ...
    // Sort logic: Frogs first, then normal tasks, then future tasks
    const sortTodos = (list: Todo[]) => [...list].sort((a, b) => {
    ...
    return (
        <div className="p-5 pb-32 space-y-6">
            {sortedObjectives.map(obj => {
                ...
                const sortedObjTodos = sortTodos(objTodos);
            ...
            {uncategorizedTodos.length > 0 && (
                <section>
                    ...
                    {sortTodos(uncategorizedTodos).map(t => renderTodoCard(t, t.isFrog))}
                </section>
            )}
  ...
  return (
    <div className="h-full flex flex-col bg-white overflow-hidden">
      ...
      <button 
        className="absolute bottom-6 right-6 z-[80] p-3 bg-stone-900 text-white rounded-full shadow-xl shadow-stone-900/30 active:scale-90 transition-all animate-in zoom-in duration-200 flex items-center justify-center hover:bg-stone-800"
        onClick={() => {
            setEditingTodo({
                id: generateId(),
                title: '',
