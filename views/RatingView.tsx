  ...
  return (
    <div className="h-full bg-white overflow-y-auto custom-scrollbar">
      <div className="px-4 py-4 space-y-4 pb-32">
        {/* Score Dashboard - Card Shadow Added */}
        <section className="bg-primary rounded-2xl p-4 text-white flex flex-col items-center gap-3 relative overflow-hidden shadow-card">
           <div className="flex items-center justify-between w-full relative z-10">
                <div className="flex flex-col">
                    <span className="text-[9px] font-black text-white/60 uppercase tracking-[0.2em]">账户可用积分</span>
                ...
                <div className="flex gap-2">
                    <button 
                        onClick={onOpenStats} 
                        className="p-2.5 bg-white/20 hover:bg-white/30 text-white rounded-full transition-all active:scale-90" 
                        title="打分统计"
                    >
                        <TrendingUp size={18} />
                    </button>
                    <button onClick={() => { setShopTab('buy'); onToggleShop?.(true); }} className="p-2.5 bg-white/20 hover:bg-white/30 text-white rounded-full transition-all active:scale-90">
                        <ShoppingBag size={18} />
                    </button>
                </div>
           </div>
           ...
           <div className="flex items-center justify-between w-full relative z-10 px-0.5">
                <div className="flex flex-col">
                    <span className="text-[8px] font-bold text-white/60 uppercase tracking-widest">今日评估分</span>
                    <span className={cn("text-xs font-black", todayScore >= 0 ? "text-white" : "text-rose-200")}>
                        {todayScore > 0 ? `+${todayScore}` : todayScore}
                    </span>
                </div>
                <div className="flex flex-col items-end">
                    <span className="text-[8px] font-bold text-white/60 uppercase tracking-widest">累计获取</span>
                    <span className="text-xs font-black text-white/90">{lifetimeScore}</span>
                </div>
           </div>
        </section>
        ...
        <section className="space-y-3">
          <div className="flex justify-between items-center px-1">
             <h3 className="text-base font-black text-stone-900 uppercase tracking-tight leading-none">自律评估</h3>
          ...
          <div className="space-y-2">
             {ratingItems.map(item => {
                 ...
                 return (
                    <div key={item.id} className="bg-stone-50/50 rounded-xl p-3 border border-stone-100 space-y-2.5">
        ...
        <section className="space-y-2">
          <div className="flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-black text-stone-900 uppercase tracking-tight">今日复盘</h3>
              </div>
              <button 
      ...
      {isShopOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-stone-900/60 p-4">
              <div className="bg-white rounded-[1.5rem] w-full max-w-lg flex flex-col border border-stone-300 overflow-hidden max-h-[85vh] animate-in zoom-in-95 duration-200 h-full max-h-[600px]">
                  ...
                  <div className="px-5 pt-3 shrink-0">
                      <div className="flex bg-stone-100 p-1 rounded-xl">
                          <button 
                             ...
                             className={cn("flex-1 py-1.5 rounded-lg text-[10px] font-black transition-all", shopTab === 'buy' ? "bg-white text-stone-900 border border-stone-200" : "text-stone-400 border border-transparent")}
                          >
                              商品兑换
                          </button>
                          <button 
                             onClick={() => setShopTab('history')}
                             className={cn("flex-1 py-1.5 rounded-lg text-[10px] font-black transition-all", shopTab === 'history' ? "bg-white text-stone-900 border border-stone-200" : "text-stone-400 border border-transparent")}
                          >
                              兑换记录
                          </button>
                      </div>
                      ...
                      {shopTab === 'buy' ? (
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                              {shopItems.map(item => (
                                  <div key={item.id} className="relative group">
                                      <button onClick={() => !isShopEditMode && balance >= item.cost && onRedeem(item)} className={cn("w-full bg-white rounded-xl p-3 border border-stone-100 flex flex-col items-center gap-1.5 transition-all", !isShopEditMode && balance < item.cost ? "opacity-50 grayscale" : "hover:border-primary/30 active:scale-95")}>
                                      ...
                                      {isShopEditMode && (
                                          <div className="absolute -top-1.5 -right-1.5 flex gap-1 animate-in fade-in zoom-in duration-200">
                                              <button onClick={() => setEditingShopItem(item)} className="p-1.5 bg-stone-900 text-white rounded-full hover:bg-stone-800"><Edit2 size={8} /></button>
                                              <button onClick={() => onUpdateShopItems(shopItems.filter(i => i.id !== item.id))} className="p-1.5 bg-rose-500 text-white rounded-full hover:bg-rose-600"><Trash2 size={8} /></button>
                                          </div>
                                      )}
                                  </div>
                              ))}
                          ...
                          <div className="space-y-2">
                              {sortedRedemptions.length > 0 ? sortedRedemptions.map(record => {
                                  ...
                                  return (
                                      <div key={record.id} className="flex items-center justify-between p-3 bg-white border border-stone-100 rounded-xl">
      ...
      {isTemplateModalOpen && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center bg-stone-900/60 p-4">
            <div className="bg-white rounded-[1.5rem] w-full max-w-sm flex flex-col border border-stone-300 overflow-hidden max-h-[70vh] animate-in zoom-in-95 duration-200">
                        ...
                        <div key={template.id} className="group relative">
                            ...
                            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button 
                                    ...
                                    className="p-1.5 bg-white border border-stone-100 rounded-lg text-stone-400 hover:text-stone-900"
                                >
                                    <Edit2 size={12} />
                                </button>
                                {onDeleteReviewTemplate && (
                                    <button 
                                        ...
                                        className="p-1.5 bg-white border border-stone-100 rounded-lg text-stone-400 hover:text-rose-500"
                                    >
                                        <Trash2 size={12} />
                                    </button>
                                )}
      ...
      {isConfigModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-stone-900/60 p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg border border-stone-300 overflow-hidden max-h-[85vh] flex flex-col animate-in zoom-in-95 duration-200">
            <div className="px-5 py-4 bg-stone-50 border-b border-stone-100 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-2">
                ...
                {ratingItems.map(item => (
                    <div key={item.id} className="flex items-center justify-between p-3 bg-stone-50 rounded-xl border border-stone-100 hover:border-stone-200 group transition-all">
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                          <button onClick={() => setEditingItem(item)} className="p-1.5 text-stone-400 hover:text-stone-900 hover:bg-white rounded-lg"><Edit2 size={14} /></button>
                          <button onClick={() => onUpdateRatingItems(ratingItems.filter(i => i.id !== item.id))} className="p-1.5 text-stone-400 hover:text-rose-500 hover:bg-white rounded-lg"><Trash2 size={14} /></button>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/30 p-4">
              <form onSubmit={handleSaveItem} className="bg-white rounded-[1.5rem] p-5 w-full max-w-md border border-stone-200 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
                ...
                <div className="flex justify-end gap-2 pt-1">
                   ...
                   <button type="submit" className="px-6 py-2.5 bg-primary text-white rounded-xl text-[10px] font-black uppercase active:scale-95 transition-all">保存</button>
                </div>
              </form>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/30 p-4">
          <form onSubmit={handleSaveShopItem} className="bg-white rounded-[1.5rem] p-5 w-full max-w-sm border border-stone-200 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-end gap-2 pt-1">
               ...
               <button type="submit" className="px-6 py-2.5 bg-primary text-white rounded-xl text-[10px] font-black uppercase active:scale-95">确定</button>
            </div>
          </form>
        </div>
      )}
